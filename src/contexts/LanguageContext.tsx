...
const TRANSLATION_CACHE: Record<string, Translations> = {
...
export const LanguageProvider = ({ children }: { children: ReactNode }) => {
  ...
  const setLanguage = useCallback(async (lang: string) => {
    ...
    if (TRANSLATION_CACHE[lang]) {
      setTranslations(TRANSLATION_CACHE[lang]);
      return;
    }

    setLoading(true);
    try {
      const result = await translateUI({ language: lang, texts: ENGLISH_TRANSLATIONS });
      const newTranslations = result.translations;
      TRANSLATION_CACHE[lang] = newTranslations;
      setTranslations(newTranslations);
    } catch (error) {
      console.error("Failed to translate UI:", error);
      setLanguageState('English'); // Revert to English on error
  ...
  const t = useCallback((key: string): string => {
    return translations[key] || ENGLISH_TRANSLATIONS[key] || key;
  }, [translations]);
