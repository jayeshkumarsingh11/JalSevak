...
export const LanguageProvider = ({ children }: { children: ReactNode }) => {
  ...
  const translationCache = useRef<Record<string, Translations>>({ English: ENGLISH_TRANSLATIONS });
  ...
  const setLanguage = useCallback(async (lang: string) => {
    ...
    setLoading(true);
    try {
      const response = await translateUI({
        language: lang,
        jsonToTranslate: JSON.stringify(ENGLISH_TRANSLATIONS),
      });
      
      const newTranslations = JSON.parse(response.translatedJson);
